...
結果として **知らないフラグを渡すとリンタが黙って無効になった。** 実際に問題のあるリポジトリで測った値：

```
tenken                    → exit 1（問題を検出）
tenken --zzz-not-a-flag   → exit 0（"skipping" と出て何も検査しない）
```

CI 設定にありそうな名前のフラグ（別ツールから写した、リネーム前の名前、単なる打ち間違い）が
1つ紛れ込むだけで、チェックは緑のまま永久に何も見なくなる。**緑であることが、もう誰も
見に来ない理由になる。**
...
インストールした人が最初に打つものが両方とも無く、`tenken --help` は「パスとして解釈」
